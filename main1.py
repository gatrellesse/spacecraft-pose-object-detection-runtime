    ...
    with open(os.devnull, "w") as devnull:
        ...
        for i, image_id in progress_bar:
            ...
            if len(result.boxes) > 0:
                # get categories
                detected_class = result.names[0]

                # if detect categorise of "class1"
                if detected_class == "class1":

                    # move the photo to class1
                    output_class_folder = os.path.join(output_path.parent, "class1")
                    os.makedirs(output_class_folder, exist_ok=True)
                    shutil.move(str(images_dir / f"{image_id}.png"), output_class_folder)

                    # update log
                    logger.info(f"Image {image_id} classified as {detected_class} and moved to {output_class_folder}")

            # if yolo detect no object
            else:
                # move image to "other"
                output_default_folder = os.path.join(output_path.parent, "other")
                os.makedirs(output_default_folder, exist_ok=True)
                shutil.move(str(images_dir / f"{image_id}.png"), output_default_folder)

                # update log
                logger.info(f"Image {image_id} classified as 'other' and moved to {output_default_folder}")
